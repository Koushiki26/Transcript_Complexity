...
table1 <- read.csv("sscount_low_TC_h.csv", sep = " ")
head(table1)
...
table2 <- read.csv("sscount_high_TC_h.csv", sep = " ")
head(table2)
...
tiff("/home/dell/Documents/koushiki/final_fig/fig_2b5.tiff",width = 1200, height = 1200, res = 300)
...
df
level_order <- c("AC","AG")
tiff("/home/dell/Documents/koushiki/final_fig/fig_2b6.tiff",width = 1200, height = 1200, res = 300)
p <- ggplot(df, aes(factor(ss, level=level_order), Frequency_of_introns, fill=Type_of_gene)) +
  geom_bar(stat="identity", position = "dodge") + 
  scale_fill_manual(values = c("#31A2AC","#3A5199","#EB8A3E","#CB0000"))
p + xlab("") + ylab("Fraction of introns")+ 
