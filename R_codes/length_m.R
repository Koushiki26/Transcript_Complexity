...
library(dplyr)
data_intron <- read.table("intron_length_m.txt", header = F, sep = "\t")
head(data_intron)
colnames(data_intron) <- c("Transcript_id", "intron_length")
data_exon <- read.table("exon_length_m.txt", header = F, sep = "\t")
head(data_exon)
colnames(data_exon) <- c("Transcript_id", "exon_length")
lncRNA_trns <- read.table("lncrna_transcript_id_m.txt", header = F, sep = "\t")
head(lncRNA_trns)
colnames(lncRNA_trns) <- c("gene_id", "transcript_id")
length(lncRNA_trns$transcript_id)
CDS_trns <- read.table("mrna_transcript_id_m.txt", header = F, sep = "\t")
head(CDS_trns)
...
write.csv(lncRNA_trns, file = "lncrna_trns_m.csv", col.names = T, row.names = F, na = "0")
write.csv(CDS_trns, file = "mrna_trns_m.csv", col.names = T, row.names = F, na = "0")
...
head(data_CDS)
lncRNA_trns <- read.csv("lncRNA_trns_m.csv", header = T)
head(lncRNA_trns)
length(lncRNA_trns$transcript_id)
CDS_trns <- read.csv("CDS_trns_m.csv", header = T)
head(CDS_trns)
length(CDS_trns$transcript_id)
...
write.csv(data_lncRNA, file = "lncrna_length_m.csv", col.names = T, row.names = F, na = "0")
write.csv(data_CDS, file = "mrna_length_m.csv", col.names = T, row.names = F, na = "0")