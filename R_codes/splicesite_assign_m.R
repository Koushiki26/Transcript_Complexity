...
splicesite <- read.table("splicesites_m.txt", header=F, sep="\t")
head(splicesite)
colnames(splicesite) <- c("transcript_id", "5'SS", "3'SS")
...
write.csv(splicesite, file = "lncrna_ss_m.csv", col.names = T, row.names = F)
...
splicesite <- na.omit(splicesite)
write.csv(splicesite, file = "mrna_ss_m.csv", col.names = T, row.names = F)