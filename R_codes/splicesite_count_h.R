...
lncRNA_trns <- read.csv("lncrna_ss_h.csv", header = T)
head(lncRNA_trns)
colnames(lncRNA_trns) <- c("transcript_id", "5'SS", "3'SS", "gene_id")
length(lncRNA_trns$transcript_id)
CDS_trns <- read.table("mrna_ss_h.csv", header = T, sep = ",")
head(CDS_trns)
...
write.csv(lncRNA_trns, file = "lncrna_ss_h.csv", col.names = T, row.names = F)
...
write.csv(CDS_trns, file = "mrna_ss_h.csv", col.names = T, row.names = F)
...
length(CDS_trns$transcript_id)
CDS_trns <- CDS_trns[1:148236,]

TPE <- c(lncRNA_trns$TPE, CDS_trns$TPE)
x <- 0.3333333
...
write.table(y, file = "sscount_low_TC_h.csv", col.names = T, row.names = F)
...
y <- apply(y,2,as.character)
write.table(y, file = "sscount_high_TC_h.csv", col.names = T, row.names = F)