...
for each_line in genome:
    ...
    if not ((re.search(pattern1, str(each_line))) or (re.search(pattern2, str(each_line))) or (re.search(pattern3, str(each_line))) or (re.search(pattern4, str(each_line))) or (re.search(pattern5, str(each_line)))):
        continue
    gene = trns = ''
    for elements in column[8].split(';'):
        if elements.startswith('gene_id'):
...
df_t = df.T
df_t.to_csv("lncrna_tsl5_m.csv", header = False)
