...
for each_line in genome:
    ...
    pattern3 = 'transcript_support_level 3'
    pattern4 = 'transcript_support_level 4'
    pattern5 = 'transcript_support_level 5'
    pattern6 = 'transcript_support_level NA'
    if not ((re.search(pattern1, str(each_line))) or (re.search(pattern2, str(each_line))) or (re.search(pattern3, str(each_line))) or (re.search(pattern4, str(each_line))) or (re.search(pattern5, str(each_line))) or (re.search(pattern6, str(each_line)))):
        continue
    gene = trns = ''
    for elements in column[8].split(';'):
        if elements.startswith('gene_id'):
...
df_t = df.T
df_t.to_csv("lncrna_tsl6_m.csv", header = False)
